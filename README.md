...
Start the pinned release server and frontend together with one of the bundled Cedar environments:
...
- `test:e2e:live` builds the pinned REST release source, loads real Cedar policy/schema fixtures with both upload and Bearer credentials, and runs the primary authorization flow through Vite's same-origin proxy. Set `TREETOP_SERVER_BIN` to use an explicitly built local executable.
- `test:demos` validates every advertised demo decision and principal lookup against the pinned release server.
